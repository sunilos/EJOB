...
class UserService extends BaseService {
    ...
    preload(request, response) {
        ...
        rService.search(new Role(),0, function (err, result) {
            console.log(result);
    ...
    authenticate(user, callback, ctx) {
        ...
        super.executeSQLForObject(sql, params, this.getBean(), function (err, bean) {
            if (err) {
    ...
    update(user, callback, ctx) {
        ...
        var params = [user.firstName, user.lastName, user.login,user.password, user.dob, user.mobileNo, user.roleId, user.gender, user.id];
